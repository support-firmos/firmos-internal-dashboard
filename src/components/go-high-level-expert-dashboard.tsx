...
const metrics: Metric[] = [
  { name: 'Discovery Calls Booked', formula: 'Total calls booked from funnels', benchmark: 50, unit: '' },
  { name: 'Referral Ratio', formula: '(Total Referral Calls Booked / Total Customers)', benchmark: 0.3, unit: '' },
  { name: 'Total Subscribers', formula: 'Total subscribers in funnels', benchmark: 1000, unit: '' },
  { name: 'Upsell Conversion Rate', formula: '(Upsell Conversions via Email / Total Emails Sent) × 100', benchmark: 5, unit: '%' },
  { name: 'Online Review Acquisition Rate', formula: '(Positive Reviews Acquired / Total Review Requests Sent) × 100', benchmark: 20, unit: '%' },
]
...
const generateRandomData = (startDate: Date, endDate: Date): DataPoint[] => {
  ...
  return weeks.map((week) => {
    ...
    metrics.forEach(metric => {
      ...
      } else if (metric.name === 'Referral Ratio') {
        value = Math.random() * 0.5
      } else if (metric.name === 'Total Subscribers') {
...
export function GoHighLevelExpertDashboard() {
  ...
  const renderMetricCard = (metric: Metric) => {
    ...
    const isGood = metric.name === 'Referral Ratio' 
    ...
    const shouldShowDecimals = [
      ...
      'Online Review Acquisition Rate'
  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Go High Level Expert Dashboard</h1>
