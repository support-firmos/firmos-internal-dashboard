...
const generateRandomData = (startDate: Date, endDate: Date): DataPoint[] => {
  ...
  return weeks.map((week, index) => {
    ...
    metrics.forEach(metric => {
      let value: number
      // Use previous value to generate new value within a reasonable range
      const prevValue = previousValues[metric.name] || metric.benchmark
      
      if (metric.name === 'CAC Ratio') {
        value = prevValue * (0.9 + Math.random() * 0.2) // ±10% change
      } else if (metric.name === 'Operating Cash Flow') {
        value = prevValue + (Math.random() * 200000 - 100000) // ±100k change
      } else if (metric.unit === '$') {
        value = prevValue * (0.95 + Math.random() * 0.1) // ±5% change
      } else if (metric.unit === '%') {
        value = prevValue * (0.95 + Math.random() * 0.1) // ±5% change
      } else if (metric.name === 'Time to Close (Sales Cycle Length)') {
        value = prevValue * (0.98 + Math.random() * 0.04) // ±2% change
      } else {
        value = prevValue * (0.95 + Math.random() * 0.1) // ±5% change
      }
      
      weekData[metric.name] = Number(value.toFixed(2))
      previousValues[metric.name] = value // Store for next iteration
    })
    return weekData
  })
}
